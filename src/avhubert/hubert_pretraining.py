...
DBG = False  # if len(sys.argv) == 1 else False
...
class AVHubertPretrainingConfig(FairseqDataclass):
    ...
    modalities: Optional[List[str]] = field(
        default_factory=lambda: ["audio", "video"], metadata={"help": "modalities to load"}
    )
    ...
    noise_wav: Optional[str] = field(
        default=None, metadata={"help": "manifest of noise wav files (one wav file path per line)"}
    )
...
class AVHubertPretrainingTask(FairseqTask):
    ...
    def load_tokenizer(self):
        bpe_args = Namespace(
            **{"bpe": self.cfg.tokenizer_bpe_name, f"{self.cfg.tokenizer_bpe_name}_model": self.cfg.tokenizer_bpe_model}
        )
    ...
    @classmethod
    def setup_task(cls, cfg: AVHubertPretrainingConfig, **kwargs) -> "AVHubertPretrainingTask":
        if cfg.pdb:
            import pdb
    ...
    def load_dataset(self, split: str, **kwargs) -> None:
        ...
        noise_fn, noise_snr = f"{self.cfg.noise_wav}/{split}.tsv" if self.cfg.noise_wav is not None else None, eval(
            self.cfg.noise_snr
        )
